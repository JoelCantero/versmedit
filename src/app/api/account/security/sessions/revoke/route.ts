...
import { revokeAccountSession } from "@/modules/account/security/service";
import {
...
const ROUTE = "/api/account/security/sessions/revoke";
const LOG_MESSAGE = "account security session revocation completed";
...
export async function POST(request: NextRequest) {
  ...
  const parsed = accountSecurityIndividualCommandSchema.safeParse(body);
  if (!parsed.success) {
  ...
  if (result.status === "completed") {
    log.info(
      { outcome: ACCOUNT_SECURITY_REVOKE_SESSION_COMPLETED_OUTCOME },
      LOG_MESSAGE,
    );
  ...
  log.warn(
    { outcome: ACCOUNT_SECURITY_REVOKE_SESSION_FAILED_OUTCOME },
    LOG_MESSAGE,
  );
